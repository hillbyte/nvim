...
return {
	-- coc.nvim plugin
	{
		...
		config = function()
			...
			--    ]])
			--
			-- Set up coc extensions with optimization
			vim.g.coc_global_extensions = {
				"coc-json",
				"coc-tsserver",
				"coc-html",
				...
				"coc-eslint",
				"coc-prettier",
				-- Only load the extensions you need, consider disabling "coc-snippets" if not essential
			}

			-- Reduce memory usage for tsserver
			vim.cmd([[
				augroup CocTsServerOptimization
					autocmd!
					autocmd FileType typescript,json let g:coc_node_args=['--max-old-space-size=200']
				augroup END
			]])

			-- Disable diagnostics for better performance
			vim.g.coc_diagnostic_enable = 0

			-- Disable automatic formatting
			vim.g.coc_enable_format_on_save = 0
		end,
	},
}
